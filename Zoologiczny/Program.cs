					...
					try{
						switch(view.EnterIntNumber()){
		        				...
		        				view.DisplayAvailableAnimals();
		        					switch(view.EnterIntNumber()){
		        						case(0): // add doge
		        							...
		        							try{
		        								// dog builder
			        							AnimalBreeder<DogBuilder<Dog>> animalBreeder = new AnimalBreeder<DogBuilder<Dog>>();
			        							animalBreeder.AnimalBuilder = new DogBuilder<Dog>();
			        							animalBreeder.AnimalBuilder.CreateNewAnimal();
			        							animalBreeder.ConstructAnimal(view.EnterIntNumber(), view.EnterDoubleNumber());
			        							
			        							//controller.AddAnimalToWarehouse(animalBreeder.Animal);
			        							animalBreeder = null;
		        			...
		        			case 5: // Display client's basket
		        				view.DisplayClientStatus(model.Client.List, model.Client.Sum);
		        				view.WaitAndClear();
		        				break;	
		        			case 6: // remove number of animals form client's basket
		        				view.DisplayClientStatus(model.Client.List);
		        				try{
